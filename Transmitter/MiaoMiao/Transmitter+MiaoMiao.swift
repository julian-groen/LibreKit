...
public class MiaoMiaoTransmitter: Transmitter {
    ...
    public var name: String = "MiaoMiao"

    public var manufacturer: String = "Tomato"

    public var notifyCharacteristic: CBUUID = CBUUID(string: "6E400003-B5A3-F393-E0A9-E50E24DCCA9E")

    public var serviceCharacteristics: [CBUUID] = [CBUUID(string: "6E400001-B5A3-F393-E0A9-E50E24DCCA9E")]
    ...
    required init(with identifier: String) {
        ...
        os_log("MiaoMiaoTransmitter initialized.", log: TransmitterManager.log, type: .default)
    }
    
    public func requestData(_ peripheral: CBPeripheral) -> Bool {
        return writeValueToPeripheral(peripheral, value: Data.init([0xF0]), type: .withoutResponse)
    }
    ...
    public func updateValueForNotifyCharacteristic(_ peripheral: CBPeripheral, value: Data) {
        ...
        guard let firstByte = rxBuffer.first, let response = MiaoMiaoResponseType(rawValue: firstByte) else {
            reset()
            return
        }
        ...
        case .dataPacket:
            if rxBuffer.count >= 363 && rxBuffer.last == 0x29 {
                let data = SensorData(bytes: rxBuffer.subdata(in: 18..<362), timestamp: Date().timeIntervalSince1970)
                
                if let sensorData = data, sensorData.hasValidCRCs {
        ...
        case .frequencyChanged:
            os_log("Transmitter recieved frequency changed response, not really meant to happen?", log: TransmitterManager.log, type: .default)
            reset()
        case .newSensor:
            confirmSensor(peripheral)
            reset()
        case .noSensor:
            NotificationManager.sendSensorNotDetectedNotificationIfNeeded()
            reset()
        }
    }
    
    private func confirmSensor(_ peripheral: CBPeripheral) {
        NotificationManager.sendSensorChangeNotificationIfNeeded()
        _ = writeValueToPeripheral(peripheral, value: Data.init([0xD3, 0x01]), type: .withoutResponse)
    }
