...
fileprivate enum ResponseType: UInt8 {
    case sensorPacket       = 0x28
    case newSensorFound     = 0x32
    case noSensorFound      = 0x34
...
public class MiaoMiaoTransmitter: Transmitter {
    ...
    private func writeValueToPeripheral(value: Data) -> Bool {
...
extension MiaoMiaoTransmitter: CBPeripheralDelegate {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in service.characteristics ?? [] {
            if characteristic.properties.intersection(.notify) == .notify && characteristic.uuid == notifyCharacteristic {
                peripheral.setNotifyValue(true, for: characteristic)
            }
            if characteristic.properties.intersection(.write) == .write && characteristic.uuid == writeCharacteristic {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if writeValueToPeripheral(value: Data.init([0xF0])) { log.info("Requested data from transmitter") }
    }
    
    public func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        log.default("%@: %@: %@", #function, characteristic, String(describing: error))
        
        if let value = characteristic.value, characteristic.uuid == notifyCharacteristic {
            if timestampLastPacket.addingTimeInterval(maxWaitForNextPacket - 1) < Date() {
                self.resetDataBuffer()
            }
            rxBuffer.append(value)
           
            guard let byte = rxBuffer.first, let response = ResponseType(rawValue: byte) else {
                self.resetDataBuffer(); return
            }
            handleUpdatedNotificationValue(responseType: response)
        }
    }
}
