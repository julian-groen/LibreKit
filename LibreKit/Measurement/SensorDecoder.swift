...
public class SensorCRC {
    ...
    public static func parse(data: Data) -> Bool {
        ...
        return SensorCRC.hasValidCrc([UInt8] (data.subdata(in: header))) &&
            SensorCRC.hasValidCrc([UInt8] (data.subdata(in: body))) &&
            SensorCRC.hasValidCrc([UInt8] (data.subdata(in: footer)))
    }
    
    public static func hasValidCrc(_ bytes: [UInt8]) -> Bool {
        let calculatedCrc = SensorCRC.crc16(Array(bytes.dropFirst(2)), seed: 0xffff)
